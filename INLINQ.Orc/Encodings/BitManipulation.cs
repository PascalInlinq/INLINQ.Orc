...
namespace INLINQ.Orc.Encodings
{
    ...
    {
        internal class StreamIndex
        {
            internal uint theStreamIndex;
        }
        ...
        public static long ReadLongBE(this byte[] stream, int numBytes, ref uint streamIndex)
        {
            long result = 0;
        ...
        public static void WriteLongBE(this Span<byte> stream, ref int streamIndex, int numBytes, long value)
        {
            for (int i = numBytes - 1; i >= 0; i--)
            {
                byte nextByte = (byte)(((ulong)value) >> (i * 8));
                stream[streamIndex++] = nextByte;
            }
        }
        ...
        public static Tuple<int, int[]> GenerateHistogramOfBitWidths(this IEnumerable<long> values)
        {
            ...
            {
                int numBits = NumBits((ulong)value);
                int nearestWidth = FindNearestDirectWidth(numBits);
                int encodedNumBits = nearestWidth.EncodeDirectWidth();
        ...
        public static Tuple<int, int[]> GenerateHistogramOfBitWidths(this ReadOnlySpan<long> values)
        {
            int[]? histogram = new int[32];
            int totalNumValues = 0;
            foreach (long value in values)
        ...
        {
            int totalNumValues = histogramTuple.Item1;
            int[]? histogram = histogramTuple.Item2;
        ...
        public static IEnumerable<long> ReadBitpackedIntegers(this byte[] stream, int bitWidth, int count)
        {
            StreamIndex streamIndex = new StreamIndex();
            return ReadBitpackedIntegers(stream, streamIndex, bitWidth, count);
        }

        internal static IEnumerable<long> ReadBitpackedIntegers(this byte[] stream, StreamIndex streamIndex, int bitWidth, int count)
        {
            byte currentByte = 0;
            int bitsAvailable = 0;
            uint index = streamIndex.theStreamIndex;
            for (int i = 0; i < count; i++)
            {
                ulong result = 0;
                int neededBits = bitWidth;
            ...
            streamIndex.theStreamIndex = index;
        }
        ...
        public static void WriteBitpackedIntegers(this Span<byte> stream, ref int streamIndex, IEnumerable<long> values, int bitWidth)
        {
            byte currentByte = 0;
            int bitsAvailable = 8;
            foreach (long value in values)
            {
                ...
                while (bitsToWrite > bitsAvailable)
                {
                    currentByte |= (byte)(((ulong)value) >> (bitsToWrite - bitsAvailable));
                    bitsToWrite -= bitsAvailable;
                    stream[streamIndex++] = currentByte;
                    currentByte = 0;
                    bitsAvailable = 8;
                }
                bitsAvailable -= bitsToWrite;
        ...
        public static void WriteBitpackedIntegersSpan(this Span<byte> stream, ref int streamIndex, ReadOnlySpan<long> values, int bitWidth)
        {
            byte currentByte = 0;
            int bitsAvailable = 8;
            foreach (long value in values)
        ...
        public static long ReadVarIntUnsigned(this byte[] stream, ref uint streamIndex)
        {
            long result = 0;
            long currentByte;
            int bitCount = 0;
        ...
        public static void WriteVarIntUnsigned(this Stream stream, long value)
        {
            while (true)
            {
                ...
                {
                    long curByte = (value & 0x7f) | 0x80;        //Set the high bit--we have more bytes coming
        ...
        public static void WriteVarIntUnsigned(this Span<byte> stream, ref int streamIndex, long value)
        {
            while (true)
            {
                if ((value & ~0x7fL) == 0)
                {
                    stream[streamIndex++] = (byte)value;
                    break;
                }
                else
                {
                    long curByte = (value & 0x7f) | 0x80;        //Set the high bit--we have more bytes coming
                    stream[streamIndex++] = (byte)curByte;
        ...
        //{
        //    long unsigned = ReadVarIntUnsigned(stream);
        //    return unsigned.ZigzagDecode();
        //}

        public static long ReadVarIntSigned(this byte[] stream, ref uint streamIndex)
        {
            long unsigned = ReadVarIntUnsigned(stream, ref streamIndex);
            return unsigned.ZigzagDecode();
        }

        public static void WriteVarIntSigned(this Stream stream, long value)
        {
            long encodedValue = value.ZigzagEncode();
            stream.WriteVarIntUnsigned(encodedValue);
        }

        public static void WriteVarIntSigned(this Span<byte> stream, ref int streamIndex, long value)
        {
            long encodedValue = value.ZigzagEncode();
        ...
        {
            bool noOverflow = (left ^ right) >= 0 || (left ^ (left - right)) >= 0;
            return !noOverflow;
        }
    }
}
